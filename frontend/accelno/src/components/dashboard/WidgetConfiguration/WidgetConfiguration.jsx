...
const WidgetConfiguration = ({ widgetId, setInput, setSize, size, handleSaveBtnClick, setEdit, screen }) => {
		...
		<div className="py-3 px-2 space-y-2">
				...
				<div className="flex justify-center items-center gap-4 text-xs font-semibold ">
					<button
						...
						className={`rounded-full py-2 px-3 border border-dashboardBlue hover:bg-dashboardBlue hover:text-white ${
							size === 'small' ? 'bg-dashboardBlue text-white' : 'bg-white  text-dashboardBlue'
						}`}
					>
						{' '}
						Small{' '}
			...
			<div className="flex justify-center items-center space-x-4 py-5">
				<button
					...
					className="flex justify-center items-center py-2 w-16 bg-dashboardBlue  text-white text-xs font-semibold gap-1 rounded-lg"
				>
					{' '}
					Save
				</button>
